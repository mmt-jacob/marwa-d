...
color and weight of the lines (if any), and the font, alignment and padding of the strings.
...
NOTE: THIS HAS BEEN MODIFIED TO INCLUDE TITLES BEFORE TABLES ON THE FIRST PAGE.
"""
...
from reports.elements.styles import Styles
...
class TitleTable(Flowable):

    global table_page_count
    ...
    def __init__(self, data, colWidths=None, rowHeights=None, style=None,
                 ...
                 title=None, no_space=True, passes=0):
        global table_page_count
        self.title = title
        self.ident = ident
        self.canvas_add = TITLE_HEIGHT if self.title else 0
        self.no_space = no_space
        self.counted_page = False
        self.last_build_pass = passes
    ...
    def _calc(self, availWidth, availHeight):
        ...
        # Adjust top of table of later pages down to make space for title
        # if not self.no_space:
        #     for idx in range(0, len(self._rowpositions)):
        #         self._rowpositions[idx] -= TITLE_HEIGHT * table_page_count
    ...
    def wrap(self, availWidth, availHeight):
        ...
        # Reset counted bool on subsequent passes
        passes = self.canv._doctemplate.passes
        if self.last_build_pass != passes:
            self.last_build_pass = passes
            self.counted_page = False

        # Clear page count if no title available
        if not self.title:
            table_page_count = 0
            self.counted_page = True

        # Determine y-shift to allow for title to fit before table
        if self.title and not self.no_space and not self.counted_page:
            self.counted_page = True
            table_page_count += 1

        self._calc(availWidth, availHeight)
        self.availWidth = availWidth
        return (self._width, self._height + self.canvas_add)
    ...
    def _splitRows(self, availHeight):
        ...
        if n <= (repeatRows if isinstance(repeatRows, int) else (max(repeatRows) + 1)):
            return []
        ...
        # Prevent sliver tables after first table with title
        spaceAfter = 400 if self.title else None
        ...
        R0 = self.__class__(data[:n], colWidths=self._colWidths, rowHeights=splitH[:n],
                            ...
                            ident=ident, spaceAfter=spaceAfter,
                            spaceBefore=getattr(self, 'spaceBefore', None),
                            longTableOptimize=lto, title=self.title, passes=self.last_build_pass,
        ...
        if repeatRows:
            ...
            R1 = self.__class__(iRows + data[n:], colWidths=self._colWidths,
                                ...
                                longTableOptimize=lto, passes=self.last_build_pass, no_space=False
                                )
        ...
        R0.counted_page = True
        R1.counted_page = True
    ...
    def split(self, availWidth, availHeight):
        ...
        # Reduce available space to fit title
        # availHeight = max(0, availHeight - TITLE_HEIGHT * table_page_count)
        availHeight = max(0, availHeight - TITLE_HEIGHT)
    ...
    def draw(self):
        ...
        if self.title:
            c = self.canv
            p = Paragraph(self.title, Styles.subtitle)
            p.wrap(7.5*inch, 10*inch)
            height = self._height + 23
            self.title_space = 0
            # Adjust title between tables
            # if not self.no_space:
            #     height -= table_page_count * TITLE_HEIGHT
            p.drawOn(c, 0, height)

        # Reset table per page count
        if not self.title:
            table_page_count = 0
